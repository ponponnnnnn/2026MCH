...
└── tests/
    ...
    └── genai-eval-plan.md                   Gen-AI 評估測試計畫 T1-T11（只有計畫，尚未執行）
```
...
8. **Gen-AI 評估測試計畫**（`tests/genai-eval-plan.md`）——要向評審證明系統可靠，需要固定測試集、可重跑的評分、一張數字表。列出 T1-T11 十一項測試（log_health 抽取、raise_alert 分級與延遲、F11 個人化語域、晚報與通話摘要忠實度、語音層、模擬長輩盲測、規則遵循、A/B 對照、安全抽測、人工聆聽、清單外擔憂與記憶工具），每項寫明資料、跑法、指標與方法依據。**目前只有計畫，沒有執行，沒有任何結果數字。** 文獻調查原訂 10 個面向只完成 7 個，其中 3 個經第二層查證，文件內有誠實標示。
